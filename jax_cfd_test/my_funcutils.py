...
def trajectory(
  ...
  def step(carry_in, _):
    state, time = carry_in
    new_state, new_time = step_fn(state, time)
    carry_out = (new_state, new_time)
    frame = post_process(carry_in if start_with_input else carry_out)
    return carry_out, frame

  def multistep(initial_state, initial_time):
      trajectory = scan(
      step, 
...
"""  def step(carry_in, _):
    ori_v, state, time = carry_in
    new_state, new_time = step_fn(state, time)
    carry_out = (ori_v, new_state, new_time)
    frame = post_process(carry_in if start_with_input else carry_out)
    return carry_out, frame

  def multistep(initial_v, initial_state, initial_time):
      trajectory = scan(
      step, 
      (initial_v, initial_state, initial_time), 
      xs=None, length=steps
  )"""