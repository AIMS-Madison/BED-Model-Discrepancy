...
def square_force(
  ...
  exponent_internal = (x-center_x)**2.0+(y-center_y)**2.0
  factor = 2.0 / (2.0 * jnp.pi * 0.05 **2)
  array_square = factor * (1.0 -  exponent_internal)

  G1 = grids.GridArray(array_square, offsets[1], grid)
  G2 = grids.GridArray(array_square, offsets[0], grid)#这里的offset设计是没有道理的
...
def reciprocal_force(
  ...
  #
  #exponent_internal = (x-center_x)**2.0+(y-center_y)**2.0
  #factor = 2.0 / (2.0 * jnp.pi * 0.05 **2.0)
  #array_reciprocal = factor / (exponent_internal / (2.0 * 0.05 **2.0)+ 1.0)
  ...
  G1 = grids.GridArray(array_reciprocal, offsets[1], grid)
  G2 = grids.GridArray(array_reciprocal, offsets[0], grid)
...
def exponential_force_with_nn(
  ...
  #指数场
  exponent_internal = ((x-center_x)**2.0+(y-center_y)**2.0) / (2.0 * 0.05 **2)
  factor = 1.0 / (2.0 * jnp.pi * 0.05 **2)
  array_exp = factor * jnp.exp(-exponent_internal)
  ...
  #center_x_col = jnp.full((x_reshaped.shape[0], 1), center_x)
  #center_y_col = jnp.full((y_reshaped.shape[0], 1), center_y)
  #xy_combined = jnp.concatenate((x_reshaped, y_reshaped, center_x_col, center_y_col), axis=1)
  
  array_nn = model.apply(params,xy_combined).reshape(grid.shape) * 50.0

  G1 = grids.GridArray(array_exp+array_nn, offsets[1], grid)
  G2 = grids.GridArray(array_exp+array_nn, offsets[0], grid)#这里的offset设计是没有道理的
...
def square_force_with_nn(
  ...
  G1 = grids.GridArray(array_square+array_nn, offsets[1], grid)
  G2 = grids.GridArray(array_square+array_nn, offsets[0], grid)
