...
def implicit_convection_diffusion(
    density: float,
    viscosity: float,
    dt: float,
    grid: grids.Grid,
  ...
  if convect is None:
    def convect(v,G):  # pylint: disable=function-redefined
      return tuple(
          advection.advect_van_leer_using_limiters(u, G, dt) for u in v)
...
# change advection inside
def implicit_convection_diffusion2(
    density: float,
    viscosity: float,
    dt: float,
    grid: grids.Grid,
  ...
  if convect is None:
    def convect(v,G):  # pylint: disable=function-redefined
      return advection.advect_van_leer(G, v, dt)#_using_limiters
...
def implicit_convection_diffusion6(
    density: float,
    viscosity: float,
    dt: float,
...
def implicit_convection_diffusion7(
    density: float,
    viscosity: float,
    dt: float,
    custom_convection_velocity_k1: float,
    custom_convection_velocity_k2: float,
    custom_convection_velocity_b1: float,
    custom_convection_velocity_b2: float,
    grid: grids.Grid,
    convect: Optional[ConvectFn] = None,
    diffusion_solve: Callable = diffusion.solve_fast_diag,
    forcing: Optional[ForcingFn] = None,
  ...
  def navier_stokes_step(G: GridVariableVector, t:float):
    ...
    x_velocity_fn = lambda x, y: jnp.ones_like(x)*(custom_convection_velocity_k1*t+custom_convection_velocity_b1)
    y_velocity_fn = lambda x, y: jnp.ones_like(x)*(custom_convection_velocity_k2*t+custom_convection_velocity_b2)
    #grid1 = grids.Grid((256, 256), domain=((0, 2 * jnp.pi), (0, 2 * jnp.pi)))
    grid1 = grids.Grid((size , size), domain=((-2., 3.), (-2., 3.)))
    #grid1 = grids.Grid((size , size), domain=((-1., 2.), (-1., 2.)))
    v = initial_conditions.initial_velocity_field((x_velocity_fn, y_velocity_fn), grid1)
...
# only change adcection func
def implicit_convection_diffusion4(
    density: float,
    viscosity: float,
    dt: float,
    grid: grids.Grid,
  ...
  if convect is None:
    def convect(v,G):  # pylint: disable=function-redefined
      return advection.advect_van_leer(G, v, dt)
  ...
  def navier_stokes_step(G: GridVariableVector, t:float):
    ...
    #    types=((BCType.NEUMANN, BCType.NEUMANN), (BCType.NEUMANN, BCType.NEUMANN)),
    #  values=((None, None), (None, None))
    #)
    #velocity_bc = (bc, bc)
    #v = initial_conditions.initial_velocity_field((x_velocity_fn, y_velocity_fn), grid1, velocity_bc)
    v = initial_conditions.initial_velocity_field((x_velocity_fn, y_velocity_fn), grid1)
    ...
    dGdt = sum_fields(*accelerations)
    # Update v by taking a time step
    G = grids.GridVariable(G.array + dGdt * dt, G.bc)
    # Solve for implicit diffusion
    G = diffusion_solve(G, viscosity, dt)
    current_time = t + dt
    return G,current_time
